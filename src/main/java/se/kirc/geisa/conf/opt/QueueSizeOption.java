...
/**
 * This class handles the queue size option.
 * 
 ...
 * 
 */
public class QueueSizeOption extends AbstractOptionHandler {
	private static final long serialVersionUID = -7875114494095892495L;
	 ...
	 */
	public QueueSizeOption(Configuration configuration) {
		// Store a reference to the configuration.
		this.configuration = configuration;

		// Extract the description message for the option.
		String message = Messages.getString("options.help.queuesize");

		// Add the option.
		option = new Option("q", "queuesize", true, message);
		...
		if (cmd.hasOption(option.getOpt())) {
			...
			try {
				...
				throw new OptionHandlerException(String.format(
						Messages.getString("options.error.queuesize"),
						e.getMessage()));
			}
		}
	}
