...
/**
 * The Marker file option class in the application (driver).
 * 
 * Upgrade to Geisa
 * @author Henric Zazzi <henric@zazzi.se>
 * 
 */
public class LimitFileOption extends AbstractOptionHandler {
	 ...
	 */
	public LimitFileOption(Configuration configuration) {
		// Store a reference to the configuration.
		this.configuration = configuration;

		// Extract the description message for the option.
		String message = Messages.getString("options.help.limitfile");

		// Add the option.
		option = new Option("l", "limitfile", true, message);
	...
	public void handle(CommandLine cmd) throws OptionHandlerException {
		if (cmd.hasOption(option.getOpt())) {
			...
			try {
				configuration.setLimitFile(new File(value));
			} catch (InvalidValueException e) {
				// Non-valid values.
				throw new OptionHandlerException(String.format(
						Messages.getString("options.error.limitfile"),
						e.getMessage()));
			}
		}
	}
