...
/**
 * This class handles the data store type option.
 * 
 ...
 * 
 */
public class DataStoreTypeOption extends AbstractOptionHandler {
	private static final long serialVersionUID = 118171292041051648L;
	 ...
	 */
	public DataStoreTypeOption(Configuration configuration) {
		// Store a reference to the configuration.
		this.configuration = configuration;

		// Extract the description message for the option.
		String message = Messages.getString("options.help.datastoretype");

		// Add the option.
		option = new Option("t", "datastoretype", true, message);
		...
		if (cmd.hasOption(option.getOpt())) {
			...
			try {
				if (value.equalsIgnoreCase("m"))
					configuration.setDataStoreType(DataStoreType.MEMORY);
				else if (value.equalsIgnoreCase("f"))
					configuration.setDataStoreType(DataStoreType.FILE);
				else
					throw new OptionHandlerException(String.format(
							Messages.getString("options.error.datastoretype"),
							value));
