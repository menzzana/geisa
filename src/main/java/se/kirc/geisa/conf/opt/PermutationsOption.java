...
/**
 * This class handles the permutation count option.
 * 
 ...
 * 
 */
public class PermutationsOption extends AbstractOptionHandler {
	private static final long serialVersionUID = -4831297646412200039L;
	 ...
	 */
	public PermutationsOption(Configuration configuration) {
		// Store a reference to the configuration.
		this.configuration = configuration;

		// Extract the description message for the option.
		String message = Messages.getString("options.help.permutations");

		// Add the option.
		option = new Option("p", "permutations", true, message);
	...
	public void handle(CommandLine cmd) throws OptionHandlerException {
		if (cmd.hasOption(option.getOpt())) {
			...
			try {
				configuration.setPermutations(permutations);
			} catch (InvalidValueException e) {
				// Non-valid number.
				throw new OptionHandlerException(String.format(
						Messages.getString("options.error.permutations"),
						e.getMessage()));
			}
		}
	}
