...
/**
 * This class handles the model type option.
 * 
 ...
 * 
 */
public class ModelOption extends AbstractOptionHandler {
	private static final long serialVersionUID = -5183482640819437644L;
	 ...
	 */
	public ModelOption(Configuration configuration) {
		// Store a reference to the configuration.
		this.configuration = configuration;

		// Extract the description message for the option.
		String message = Messages.getString("options.help.modeltype");

		// Add the option.
		option = new Option("d", "model", true, message);
	...
	public void handle(CommandLine cmd) throws OptionHandlerException {
		if (cmd.hasOption(option.getOpt())) {
			...
			try {
				if (value.equalsIgnoreCase("dom"))
					configuration.setModelType(ModelType.DOMINANT);
				else if (value.equalsIgnoreCase("rec"))
					configuration.setModelType(ModelType.RECESSIVE);
				else
					throw new OptionHandlerException(String.format(
							Messages.getString("options.error.modeltype"),
							value));
