...
/**
 * This class handles the batch size option.
 * 
 ...
 * 
 */
public class BatchSizeOption extends AbstractOptionHandler {
	private static final long serialVersionUID = 5638225438149382693L;
	 ...
	 */
	public BatchSizeOption(Configuration configuration) {
		// Store a reference to the configuration.
		this.configuration = configuration;

		// Extract the description message for the option.
		String message = Messages.getString("options.help.batchsize");

		// Add the option.
		option = new Option("a", "batchsize", true, message);
		...
		if (cmd.hasOption(option.getOpt())) {
			...
			try {
				...
				throw new OptionHandlerException(String.format(
						Messages.getString("options.error.batchsize"),
						e.getMessage()));
			}
		}
	}
