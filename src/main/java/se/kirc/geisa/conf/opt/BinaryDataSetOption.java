...
/**
 * This class handles the binary data set option.
 * 
 ...
 * 
 */
public class BinaryDataSetOption extends AbstractOptionHandler {
	private static final long serialVersionUID = -2025909082497122291L;
	 ...
	 */
	public BinaryDataSetOption(Configuration configuration) {
		// Store a reference to the configuration.
		this.configuration = configuration;

		// Extract the description message for the option.
		String message = Messages.getString("options.help.binarydataset");

		// Add the option.
		option = new Option("b", "bfile", true, message);
		option.setArgName("basename");
	}
	...
	public void handle(CommandLine cmd) throws OptionHandlerException {
		if (cmd.hasOption(option.getOpt())) {
			...
			try {
				configuration.setBinaryDataSet(value);
			} catch (InvalidValueException e) {
				// Non-valid values.
				throw new OptionHandlerException(String.format(
						Messages.getString("options.error.binarydataset"),
						e.getMessage()));
			}
		}
	}
