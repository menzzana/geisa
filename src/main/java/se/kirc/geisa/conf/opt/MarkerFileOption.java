 ...
 * 
 */
public class MarkerFileOption extends AbstractOptionHandler {
	 ...
	 */
	public MarkerFileOption(Configuration configuration) {
		// Store a reference to the configuration.
		this.configuration = configuration;

		// Extract the description message for the option.
		String message = Messages.getString("options.help.markerfile");

		// Add the option.
		option = new Option("m", "markerfile", true, message);
	...
	public void handle(CommandLine cmd) throws OptionHandlerException {
		if (cmd.hasOption(option.getOpt())) {
			...
			try {
				configuration.setMarkerFile(new File(value));
			} catch (InvalidValueException e) {
				// Non-valid values.
				throw new OptionHandlerException(String.format(
						Messages.getString("options.error.markerfile"),
						e.getMessage()));
			}
		}
	}
